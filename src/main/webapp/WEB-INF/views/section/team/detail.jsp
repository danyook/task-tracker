...
<t:mainLayout title="List detail">
    ...
    <div class="container">
        <div class="content">
                    ...
                    <c:forEach var="task" items="${tasks}">
                        <li class="section-item">
                            ...
                            <c:if test="${section.type == 'TASK'}">
                                <form action="<c:url value='/task/${task.id}?section_id=${section.id}&team_id=${team_id}'/>" method="POST" class="task-form">
            ...
            <form action="<c:url value='/team-section/${section.id}?team_id=${team_id}'/>" method="POST" class="delete-form">
                <input type="hidden" name="_method" value="DELETE"/>
                <input type="submit" value="Удалить список!" class="delete-button"/>
            </form>
            <a href="${pageContext.request.contextPath}/team-section?team_id=${team_id}" class="action-link">Назад ко всем спискам</a>
        </div>
    ...
    <script>
        ...
        document.getElementById("new-task-link").addEventListener("click", function(event) {
            event.preventDefault();
            toggleForm('${pageContext.request.contextPath}/task/new?section_id=${section.id}&team_id=${team_id}', 'form-container');
        });

        document.getElementById("edit-section-link").addEventListener("click", function(event) {
            event.preventDefault();
            toggleForm('${pageContext.request.contextPath}/team-section/edit?section_id=${section.id}&team_id=${team_id}', 'form-container');
        });
        ...
        function fetchTaskDetails(taskId) {
            ...
            fetch('${pageContext.request.contextPath}/task/' + taskId + '?section_id=${section.id}&team_id=${team_id}')
                .then(response => response.text())
                .then(html => {
